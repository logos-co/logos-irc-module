# Generates headers from the IRC module plugin using logos-cpp-generator
...
pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    if [ -f "${lib}/lib/logos_irc_plugin.dylib" ]; then
      PLUGIN_FILE="${lib}/lib/logos_irc_plugin.dylib"
    elif [ -f "${lib}/lib/logos_irc_plugin.so" ]; then
      PLUGIN_FILE="${lib}/lib/logos_irc_plugin.so"
    else
      echo "Error: No logos_irc_plugin library file found"
      exit 1
    fi
    ...
    logos-cpp-generator "$PLUGIN_FILE" --output-dir ./generated_headers --module-only || {
      echo "Warning: logos-cpp-generator failed, this may be expected if the IRC module has no public API"
